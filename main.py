...
def evaluate(q_net, num_tests , n , arr_two , B , C , right_answer):
    solutions = []
    values = []
    ...
    for i_num in range(len(C)):
        heappush(stack, (C[i_num] / (sum(arr_two[j][i_num] for j in range(len(arr_two))) / sum(B) * -1.0), i_num))
    ...
    if not solutions:
        ...
        if ran < 0.3:
            ...
            return right_answer , sum(x * y for x, y in zip(right_answer, C))
...
if __name__ == '__main__':
    ...
    while die < len(arr):
        ...
        for v in prob.variables():
            print(len(result_pulp),"检查",v.varValue)
            print(v.name, '=', int(v.varValue))
            right_answer.append(int(v.varValue))
        print(right_answer)
        #print("A系数矩阵", arr_two)
        #print("B系数矩阵", B)
        #print("目标函数系数矩阵", C)
        ...
        T3 = time.time()
        solutions, values = evaluate(q_net , 10 , n , arr_two , B , C , right_answer)
        T4 = time.time()
        result_ql.append(values)
        print('Q-learning程序运行时间:%s毫秒' % ((T4 - T3) * 1000))
        print('经典程序运行时间:%s毫秒' % ((T2 - T1) * 1000))
    ...
    print(result_pulp)
    print(result_ql)
    di = sum(1 for x, y in zip(result_ql, result_pulp) if x != y)
    print("准确率：", (len(result_pulp) - di) / len(result_pulp) * 100.0, "%")
    print("总时间如下：")
    print('Q-learning程序运行时间:%s毫秒' % time2)
    print('经典程序运行时间:%s毫秒' % time1)
    # aa = [136, 137, 246, 268, 193, 169, 270, 172, 181, 188, 259, 285, 111, 166, 188, 85, 50, 246, 361, 219, 95, 199, 106,
    #  140, 103, 101, 174, 36, 207, 210, 196, 183, 273, 0, 302, 99, 212, 281, 228, 123, 98, 208, 84, 184, 393, 290, 194,
    #  297, 63, 135]
    # di = sum(1 for x, y in zip(result_ql, aa) if x != y)
    # print("准确率：", (len(result_pulp) - di) / len(result_pulp) * 100.0, "%")